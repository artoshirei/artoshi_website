...
export const ThreeDemo = () => {
  ...
  const { positionNode, colorNode, samplePosition } = useMemo(() => {
    /* Visible on dark scene bg — warm light (was near-black on black = “empty” canvas) */
    const colorNode = vec4(0.88, 0.84, 0.8, 1);
  ...
  useEffect(() => {
    ...
    /* Keep prior leftXs/textWidths until the next GPU readback so Pretext layout can
       stay on-screen (stale-while-revalidate) instead of blanking while buffers resize. */
